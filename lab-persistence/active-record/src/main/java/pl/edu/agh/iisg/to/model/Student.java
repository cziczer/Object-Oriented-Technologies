...
public class Student {
    ...
    public static Optional<Student> create(final String firstName, final String lastName, final int indexNumber) throws SQLException {
        String sql = "INSERT INTO Student (first_name, last_name, index_number) VALUES (?, ?, ?);";

        Object[] args = { firstName, lastName, indexNumber};

        try {
            var id = QueryExecutor.createAndObtainId(sql, args);
    ...
    public static Optional<Student> findByIndexNumber(final int indexNumber) {
        String sqlQuer = "SELECT * FROM Student WHERE index_number = (?);";
        return find(indexNumber, sqlQuer);
    }
    ...
    public Map<Course, Float> createReport() {
        ...
        try {
            ResultSet rs = QueryExecutor.read(sql, this.id);

            while (rs.next()) {
               var course = Course.findById(rs.getInt("course_id")).get();
               var grade = rs.getFloat("grade");
               var oldGrade = result.get(course);
               if(oldGrade != null){
                   result.replace(course, (grade + oldGrade) / 2);
               }
               else
                    result.put(course, grade);
            }
